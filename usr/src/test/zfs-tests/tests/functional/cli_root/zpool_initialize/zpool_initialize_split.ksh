...
log_must zpool split $TESTPOOL pool2 $DISK2
...
[[ "$(initialize_progress pool2 $DISK2)" -ge "$orig_prog2" ]] || \
        log_fail "Initializing lost progress on split pool"
log_mustnot eval "initialize_prog_line pool2 $DISK1 | grep suspended"
