    ...
    const zbookmark_phys_t *zb, const struct dnode_phys *dnp, void *arg)
{
	...
	record = kmem_zalloc(sizeof (*record), KM_SLEEP);
	record->object = zb->zb_object;
	record->start_blkid = start;
	record->end_blkid = (start + span < start ? 0 : start + span);
...
redact_list_cb(redact_block_phys_t *rb, void *arg)
{
	...
	data->object = rb->rbp_object;
	data->start_blkid = rb->rbp_blkid;
	data->end_blkid = rb->rbp_blkid + redact_block_get_count(rb);
...
send_range_start_compare(struct send_range *r1, struct send_range *r2)
{
	...
	if (r1->type != OBJECT && r2->type == OBJECT)
		return (1);
	if (r1->start_blkid < r2->start_blkid)
		return (-1);
	if (r1->start_blkid > r2->start_blkid)
		return (1);
	return (0);
...
find_next_range(struct send_range **ranges, bqueue_t **qs, uint64_t *out_mask)
{
	...
	if (ranges[idx]->eos_marker) {
		struct send_range *ret = kmem_zalloc(sizeof (*ret), KM_SLEEP);
		ret->eos_marker = B_TRUE;
		*out_mask = bmask;
		return (ret);
	}
	/*
	 ...
	 */
	uint64_t first_change = UINT64_MAX;
	for (i = 0; i < 3; i++) {
		if (ranges[i]->eos_marker ||
	...
	/*
	 * In this case, we need to create a new record to return that has the
	 * same properties as ranges[idx], but ends at first_change.  We also
	 * modify ranges[idx] (and every other range in bmask) to start at
	 * first_change.  If this makes any ranges be of size zero, free them,
	 * and pull the next range off the queue.
	 */
	struct send_range *ret = kmem_zalloc(sizeof (*ret), KM_SLEEP);
	*ret = *ranges[idx];
...
send_merge_thread(void *arg)
{
	...
	struct send_range *front_ranges[3];
	bqueue_t *queues[3];
	int err = 0;
	...
	for (range = find_next_range(front_ranges, queues,
	    &mask); !range->eos_marker && err == 0 && !smt_arg->cancel;
	...
	for (int i = 0; i < 3; i++) {
		...
		kmem_free(front_ranges[i], sizeof (*front_ranges[i]));
	}
	if (range == NULL)
    ...
    uint64_t blkid, blkptr_t *bp, uint32_t datablksz, struct send_range **pendp)
{
	...
	pending = kmem_zalloc(sizeof (*pending), KM_SLEEP);
	pending->object = dn->dn_object;
	pending->start_blkid = blkid;
	pending->end_blkid = blkid + 1;
	pending->type = new_type;
