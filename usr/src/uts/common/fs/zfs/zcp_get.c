    ...
    zfs_prop_t zfs_prop)
{
	...
	case ZFS_PROP_CREATION:
		...
		break;
	case ZFS_PROP_MOOCH_BYTESWAP:
		numval = dsl_get_mooch_byteswap(ds);
		break;
	...
	case ZFS_PROP_REDACT_SNAPS: {
		...
		/* push list to lua stack */
		VERIFY0(zcp_nvlist_to_lua(state, snaps, NULL, NULL));
		/* source */
		(void) lua_pushnil(state);
		nvlist_free(snaps);
		kmem_free(strval, ZAP_MAXVALUELEN);
    ...
    const char *dataset_name, const char *prop_name)
{
	...
	if (strchr(snap_name, '#') != NULL) {
		...
		error = dsl_bookmark_lookup(dp, snap_name, new, &bmp);
		if (error == 0) {
