...
/*
 * Congestion control wrapper hook functions
 */
static inline void	cc_ack_received(tcp_t *, uint32_t, int32_t, uint16_t);
void	cc_cong_signal(tcp_t *, uint32_t, uint32_t);
static inline void	cc_post_recovery(tcp_t *, uint32_t);
	...
	case CC_RTO:
		...
		if (!tcp->tcp_cwr || tcp->tcp_rexmit) {
			npkt = ((tcp->tcp_timer_backoff ?
			    ...
			    tcp->tcp_snxt - tcp->tcp_suna) >> 1) / tcp->tcp_mss;
...
cc_post_recovery(tcp_t *tcp, uint32_t seg_ack)
{
	...
	DTRACE_PROBE3(cwnd__cc__post__recovery, tcp_t *, tcp, uint32_t, old_cwnd,
	    uint32_t, tcp->tcp_cwnd);
}
